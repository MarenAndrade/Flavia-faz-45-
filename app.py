...
def guest_form():
    ...
    with st.form("rsvp_form"):
        ...
        if attending:
            ...
            notes = st.text_area(
                "Alguma restrição alimentar ou outra informação importante?",
                placeholder="Escreva aqui (opcional)", max_chars=500
            )
...
def admin_panel():
    ...
    display = display.rename(columns={"name": "Responsável", "phone": "Telefone", "adults": "Adultos", "children": "Crianças", "babies": "Bebês", "child_ages": "Idades", "notes": "Observações"})
    st.dataframe(display[["Responsável", "Telefone", "Status", "Adultos", "Crianças", "Bebês", "Total", "Idades", "Observações"]], use_container_width=True, hide_index=True)
