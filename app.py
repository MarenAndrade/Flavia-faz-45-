...
def guest_form():
    ...
    with st.form("rsvp_form"):
        ...
        if attending:
            ...
            st.caption("Até 6 anos: grátis · De 7 a 9 anos: meia · 10 anos ou mais: adulto")
            adults = count_control("Adultos (10 anos ou mais)", "adults", 1)
            children = count_control("Crianças até 6 anos (grátis)", "children", 0)
            babies = count_control("Crianças de 7 a 9 anos (meia)", "babies", 0)
            ...
            st.info(f"Resumo: **{adults} adulto(s), {children} criança(s) até 6 anos, {babies} criança(s) de 7 a 9 anos** — **Total: {total} pessoa(s)**")
...
def admin_panel():
    ...
    col2.metric("Adultos (10 anos ou mais)", int(confirmed["adults"].sum()) if len(confirmed) else 0)
    col3.metric("Crianças até 6 anos (grátis)", int(confirmed["children"].sum()) if len(confirmed) else 0)
    col4.metric("Crianças de 7 a 9 anos (meia)", int(confirmed["babies"].sum()) if len(confirmed) else 0)
    ...
    display = display.rename(columns={"name": "Responsável", "phone": "Telefone", "adults": "Adultos (10 anos ou mais)", "children": "Crianças até 6 anos (grátis)", "babies": "Crianças de 7 a 9 anos (meia)", "child_ages": "Idades", "notes": "Mensagem"})
    st.dataframe(display[["Responsável", "Telefone", "Status", "Adultos (10 anos ou mais)", "Crianças até 6 anos (grátis)", "Crianças de 7 a 9 anos (meia)", "Total", "Mensagem"]], use_container_width=True, hide_index=True)
